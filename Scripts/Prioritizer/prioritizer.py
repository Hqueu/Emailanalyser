...
class Prioritizer:
    def __init__(self, file_name):
        ...
        self.msg_term_weights = pd.read_pickle("Scripts/Prioritizer/msg_term_weights.pkl")
        with open('Scripts/Prioritizer/sbert_model.pkl','rb') as f:
            self.sbert_model = pickle.load(f)
        with open('Scripts/Prioritizer/sentence_embeddings.pkl','rb') as f:
    ...
    def rank_message(self, msg):
        ...
        try:
            bert_vec = self.sbert_model.encode(msg['content'])
            den = np.sqrt(np.einsum('ij,ij->i', self.sentence_embeddings, self.sentence_embeddings)*np.einsum('j,j', bert_vec, bert_vec))
            msg_terms_wt = np.min(self.sentence_embeddings.dot(bert_vec) / den)
        except:
            # Some subjects from the test set result in empty vocabulary
            msg_terms_wt = 1
